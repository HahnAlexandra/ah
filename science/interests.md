---
layout: page
title: Interests
description: >
  Find some impression from my field work during scientific cruises.
hide_description: true
sitemap: false
---

The central theme of my research is understanding how organisms react and adapt to their environment. This is especially relevant with regards to climate change. During my master thesis and doctoral project, I developed the copepod genus *Acartia* as a model system to answer these questions. While my master thesis was centered around seasonal thermal tolerance and the role of developmental plasticity in tracking annual temperature changes, I shifted my focus towards local adaptation and salinity during my PhD. Salinity is an often underappreciated climate stressor that is rapidly changing, and copepods like *Acartia tonsa* are the ideal organismic group to dive into questions around adaptive capacity and resilience. By integrating seascape genomics, transcriptomics, and physiological experiments on common-gardenen populations, I was able to gain valuable insights into adaptive processes and predict the species' resilience to future changes. 

My work combines ship-based sampling, with common-garden cultures, and molecular approaches. I am familiar with generating and analyzing large OMICs data sets, and enjoy a computational challenge!


Find a list of my publications [here]({{ '/science/publications/' | relative_url }})

Keep reading to get some impressions of my field and lab work.

## Scientific cruises

![Cruise](/assets/img/projects/cruise.png)

During my scientific career, I was fortunate to join several research cruises. On some of them I participated as a student learning the ropes of life on sea and getting first hand experience operating gear such as Bongo, WP2, and CTDs, doing eDNA filtration and oxygen measurements, and of course identifying zooplankton and fish larvae as part of ecological surveys. 
During my doctoral phase, participating cruises allowed me to sample copepods for my own work. All throughout the Baltic Sea, I collected life animals to start lab cultures and preserved individuals for genetic analyses. 

## Culturing

Working with copepods has taught me a lot about keeping live cultures. From monitoring culture conditions to growing micro algae for feeding, maintaining experimental cultures requires consistent work and dedication. In my case, keeping common-gardened strains from different sampling sites, allowed me to generate a fascinating physiological data set.

![Culturing](/assets/img/projects/culturing.png)

## Microscopy

In my first job at GEOMAR as a research assistant for Dr. Catriona Clemmesen-Bockelmann, I analyzed zooplankton samples for a Baltic time series. These first experiences in copepod identification were what ultimately led me down the research path I would take in the coming years and to the thousands of copepods I would see through the lens of a binocular.

![Microscopy](/assets/img/projects/microscopy.png)

